...
test_that("compute_tests() Type II works with one predictor on mu", {
  ...
  LRTs_2 <- compute_tests(fit,
                         type = 3,
                         test_intercepts = F)
  expect_equal(as.numeric(LRTs_2$LRT_results[, 4]), chi_squares_one_pred_mu_2[3], tolerance = 1e-4)

  # Type 3 - test_intercepts = T
  LRTs_3_intercepts <- compute_tests(fit,
                                    type = 3,
                                    test_intercepts = T)
  expect_equal(as.numeric(LRTs_3_intercepts$LRT_results[, 4]), chi_squares_one_pred_mu_3, tolerance = 1e-4)

  # Type 3 - test_intercepts = F
  LRTs_3_intercepts <- compute_tests(fit,
                                    type = 3,
                                    test_intercepts = F)
  expect_equal(as.numeric(LRTs_3_intercepts$LRT_results[, 4]), chi_squares_one_pred_mu_3[3], tolerance = 1e-4)
...
test_that("compute_tests() Type II works with one predictor on mu and lambda", {
  ...
  LRTs_2_intercepts <- compute_tests(fit,
                                    type = 2,
                                    test_intercepts = T)
  expect_equal(as.numeric(LRTs_2_intercepts$LRT_results[, 4]), chisquares_one_factor_2, tolerance = 1e-4)
  ...
  LRTs_3_intercepts <- compute_tests(fit,
                                    type = 3,
                                    test_intercepts = T)
  expect_equal(as.numeric(LRTs_3_intercepts$LRT_results[, 4]), chisquares_one_factor_3, tolerance = 1e-4)

  # Type 3 - test_intercepts = F
  LRTs_3_intercepts <- compute_tests(fit,
                                    type = 3,
                                    test_intercepts = F)
  expect_equal(as.numeric(LRTs_3_intercepts$LRT_results[, 4]), chisquares_one_factor_3[c(2, 4)], tolerance = 1e-4)
...
test_that("compute_tests() works with a standard two-factorial design", {
  ...
  fit_LRTs <- compute_tests(fit,
                       type = 2,
                       test_intercepts = T)
  expect_equal(chisquares_two_factors_2, as.numeric(fit_LRTs$LRT_results[, 4]), tolerance = 1e-4)
  # check dfs
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[1]]), stats::df.residual(fit$fit_obj) + 4)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[2]]), stats::df.residual(fit$fit_obj) + 2)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[3]]), stats::df.residual(fit$fit_obj) + 2)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[4]]), stats::df.residual(fit$fit_obj) + 1)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[5]]), stats::df.residual(fit$fit_obj) + 4)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[6]]), stats::df.residual(fit$fit_obj) + 2)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[7]]), stats::df.residual(fit$fit_obj) + 2)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[8]]), stats::df.residual(fit$fit_obj) + 1)

  # Type II, test_intercepts = F
  fit_LRTs <- compute_tests(fit,
                       type = 2)
  expect_equal(chisquares_two_factors_2[-c(1, 5)], as.numeric(fit_LRTs$LRT_results[, 4]), tolerance = 1e-4)

  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[1]]), stats::df.residual(fit$fit_obj) + 2)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[2]]), stats::df.residual(fit$fit_obj) + 2)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[3]]), stats::df.residual(fit$fit_obj) + 1)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[4]]), stats::df.residual(fit$fit_obj) + 2)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[5]]), stats::df.residual(fit$fit_obj) + 2)
  ...
  fit_LRTs <- compute_tests(fit,
                       type = 3,
                       test_intercepts = F)
  expect_equal(chisquares_two_factors_3[-c(1, 5)], as.numeric(fit_LRTs$LRT_results[, 4]), tolerance = 1e-3)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[1]]), stats::df.residual(fit$fit_obj) + 1)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[2]]), stats::df.residual(fit$fit_obj) + 1)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[3]]), stats::df.residual(fit$fit_obj) + 1)
  expect_equal(stats::df.residual(fit_LRTs$reduced_fits[[4]]), stats::df.residual(fit$fit_obj) + 1)
...
test_that("compute_tests() works for factors with > 2 levels", {
  ...
  LRTs_2_intercepts <- compute_tests(fit,
                                    type = 2,
                                    test_intercepts = T)
  expect_equal(as.numeric(LRTs_2_intercepts$LRT_results[, 4]), chisquares_contingencies_2, tolerance = 1e-4)

  # Type 2 - test_intercepts = F
  LRTs_2 <- compute_tests(fit,
                         type = 2,
                         test_intercepts = F)
  expect_equal(as.numeric(LRTs_2$LRT_results[, 4]), chisquares_contingencies_2[c(2, 4)], tolerance = 1e-4)

  # Type 3 - test_intercepts = T
  LRTs_3_intercepts <- compute_tests(fit,
                                    type = 3,
                                    test_intercepts = T)
  expect_equal(as.numeric(LRTs_3_intercepts$LRT_results[, 4]), chisquares_contingencies_3, tolerance = 1e-4)
...
test_that("compute_tests() Type II works with one predictor on mu and lambda and crossed random effects", {
  ...
  LRTs_2 <- compute_tests(fit,
                         type = 2,
                         test_intercepts = F)
  expect_equal(as.numeric(LRTs_2$LRT_results[, 4]), chisquares_cross_2[c(2, 4)], tolerance = 1e-3)

  # Type 3 - test_intercepts = T
  LRTs_3_intercepts <- compute_tests(fit,
                                    type = 3,
                                    test_intercepts = T)
  expect_equal(as.numeric(LRTs_3_intercepts$LRT_results[, 4]), chisquares_cross_3, tolerance = 1e-3)

  # Type 3 - test_intercepts = F
  LRTs_3 <- compute_tests(fit,
                          type = 3,
                          test_intercepts = F)
  expect_equal(as.numeric(LRTs_3$LRT_results[, 4]), chisquares_cross_3[c(2, 4)], tolerance = 1e-3)
...
test_that("compute_tests() works for testing random effects", {
  ...
  expect_equal(unlist(lrts_test$LRT_results[, 4]), chi_squares_rdm_intercepts)
  expect_equal(unlist(lrts_test$LRT_results[, 5]), pchisqmix(chi_squares_rdm_intercepts, 2, 0.5, lower.tail = F))
  ...
  lrts_test <- compute_tests(fit, test_intercepts = T, test_ran_ef = T)
  expect_equal(unlist(lrts_test$LRT_results[, 4]), chi_squares_rdm_intercepts_uncor)
  expect_equal(unlist(lrts_test$LRT_results[, 5]), pchisqmix(chi_squares_rdm_intercepts_uncor, 1, 0.5, lower.tail = F))
})
...
test_that("compute_tests() works for testing crossed random effects", {
  ...
  fit <- fit_mesdt(~ committee + (1 | id) + (1 | file_name), ~ committee + (committee | id), dv = "assessment", data = dat_exp_2,
                   trial_type = "status_fac")
  lrts_test <- compute_tests(fit, test_intercepts = T, test_ran_ef = T)
  expect_equal(unlist(lrts_test$LRT_results[, 4]), chi_squares_rdm_cross, tolerance = 1e-5)
  expect_equal(unlist(lrts_test$LRT_results[1:3, 5]), pchisqmix(chi_squares_rdm_cross[1:3], 3, 0.5, lower.tail = F))
  expect_equal(as.numeric(lrts_test$LRT_results[4, 5]), pchisqmix(chi_squares_rdm_cross[4], 1, 0.5, lower.tail = F))

  # without correlations
  fit <- fit_mesdt(~ committee + (1 | id) + (1 | file_name), ~ committee + (committee || id), dv = "assessment", data = dat_exp_2,
                   trial_type = "status_fac")
  lrts_test <- compute_tests(fit, test_intercepts = T, test_ran_ef = T)
  expect_equal(unlist(lrts_test$LRT_results[, 4]), chi_squares_rdm_cross_uncor, tolerance = 1e-5)
  expect_equal(unlist(lrts_test$LRT_results[1:3, 5]), pchisqmix(chi_squares_rdm_cross_uncor[1:3], 1, 0.5, lower.tail = F))
  expect_equal(as.numeric(lrts_test$LRT_results[4, 5]), pchisqmix(chi_squares_rdm_cross_uncor[4], 1, 0.5, lower.tail = F))
...
test_that("compute_tests() works for testing crossed random effects without the intercept", {
  ...
  options("mesdt.backend" = "glmmTMB")
  # with correlations
  fit <- fit_mesdt(~ committee + (1 | id) + (1 | file_name), ~ committee + (committee | id), dv = "assessment", data = dat_exp_2,trial_type = "status_fac")
  lrts_test <- compute_tests(fit, test_intercepts = F, test_ran_ef = T)
  expect_equal(unname(unlist(lrts_test$LRT_results[, 4])), chi_squares_rdm_cross[2], tolerance = 1e-5)

  # without correlations
  fit <- fit_mesdt(~ committee + (1 | id) + (1 | file_name), ~ committee + (committee || id), dv = "assessment", data = dat_exp_2,
                   trial_type = "status_fac")
  lrts_test <- compute_tests(fit, test_intercepts = F, test_ran_ef = T)
  expect_equal(unname(unlist(lrts_test$LRT_results[, 4])), chi_squares_rdm_cross_uncor[2], tolerance = 1e-5)
...
test_that("compute_tests() works for only tests on mu", {
  ...
  expect_equal(chisquares_two_factors_3[c(4)], as.numeric(LRTs$LRT_results[, 4]), tolerance = 1e-4)
  ...
  expect_equal(chisquares_two_factors_2[5:8], as.numeric(LRTs$LRT_results[, 4]), tolerance = 1e-4)
})
...
test_that("compute_tests() works for only tests on lambda", {
  ...
  expect_equal(chisquares_two_factors_3[c(1, 2)], as.numeric(LRTs$LRT_results[, 4]), tolerance = 1e-4)
