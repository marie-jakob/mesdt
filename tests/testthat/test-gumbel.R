...
test_that("gumbel-max produces reasonable results.", {
  ...
  gumbel_max_mod <- fit_mesdt(~ 1,
                              ~ 1,
                              dv = "assessment", trial_type = "status",
                              data = debi3, distribution = "gumbel-max")
  ...
  expect_equal(unname(coef(gumbel_max_mod$fit_obj)[1]),
               bias * -1)
  s <- summary(gumbel_max_mod)

  expect_equal(s$d_coef[1], sens, tolerance = 1e-3)
  expect_equal(s$c_coef[1], bias, tolerance = 1e-3)
...
test_that("gumbel-max works with predictors.", {
  ...
  gumbel_max_mod <- fit_mesdt(~ 1 + committee,
                              ~ 1 + committee,
                              dv = "assessment", trial_type = "status",
                              data = debi3, distribution = "gumbel-max")

  em_c <- data.frame(emmeans(gumbel_max_mod, ~ committee, dpar = "response bias"))
  ...
  expect_equal(sens, em_d[2, 2])
  expect_equal(bias, em_c[2, 2])
})
