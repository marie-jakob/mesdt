...
test_that("compute_tests() computes the correct Chisq value for correlated random effects.", {
  ...
  suppressWarnings(lrts_test <- compute_tests(fit, test_intercepts = T))

  # Chisq values
  # low tolerance because the package function fits with nAGQ = 0 (afex with nAGQ = 1)
  expect_equal(unname(unlist(lrts_test$LRT_results[, 4])), model_test_afex$anova_table$Chisq, tolerance = 1e-4)

  expect_equal(unname(unlist(lrts_test$LRT_results[, 5])), model_test_afex$anova_table$`Pr(>Chisq)`, tolerance = 1e-4)
})


test_that("compute_tests() computes the correct Chisq value for uncorrelated random effects.", {
  # Same for the uncorrelated model
  suppressWarnings(fit <- fit_mesdt(~ x1 + (x1 || ID), ~ x1 + (x1 || ID), dv = "y",
...
test_that("compute_tests() computes the correct Chisq value for correlated random effects.", {
  ...
  suppressWarnings(fit <- fit_mesdt(~ x1 + (x1 | ID), ~ x1 + (x1 | ID), dv = "y", data = internal_sdt_data,
                   trial_type = "trial_type_fac",
                   control = lme4::glmerControl(optCtrl = list(maxfun = 12345))))
  suppressWarnings(lrts_test <- compute_tests(fit, test_intercepts = F,
                             control = lme4::glmerControl(optCtrl = list(maxfun = 12345))))
  expect_equal(lrts_test$reduced_fits$x1_lambda@optinfo$control$maxfun, 12345)
